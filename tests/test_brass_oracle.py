...
def test_windrawGNT(chain):
    ...
    chain.wait.for_receipt(
        gntw.transact({'from': owner_addr}).transfer(gntw.address, gntw_balance))
...
def test_reimburse(chain):
    ...
    with pytest.raises(TransactionFailed):
        chain.wait.for_receipt(
            cdep.transact({'from': other_addr}).reimburse(owner_addr, other_addr, reimb_size))
    assert amnt == gntw.call().balanceOf(cdep.address)
    # oracle
    chain.wait.for_receipt(
        cdep.transact({'from': oracle_addr}).reimburse(owner_addr, other_addr, reimb_size))
